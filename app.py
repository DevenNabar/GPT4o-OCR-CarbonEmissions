...
def analyze_image(image_data: str) -> str:
    ...
    **Instructions**:
    1. **Identify the Food Items**:
       - Determine if each food item is vegan or non-vegan.
       - List each food item with its classification (vegan/non-vegan).
       - List the allergens present in each food item.

    2. **Calculate Carbon Emissions**:
       - Use North-America-specific carbon emission factors to calculate the carbon emissions for each food item.
    ...
    4. **Summarize the Results in brief**:
       ...
       - Mention the type (vegan/non-vegan).
